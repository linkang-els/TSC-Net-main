...
class ANETdetection(object):
    ...
    def wrapper_compute_average_precision(self):
        ...
        get_high_tiou_predictions
        high_tiou_predictions = Parallel(n_jobs=3)(
            delayed(get_high_tiou_predictions)(                ground_truth=ground_truth_by_label.get_group(cidx).reset_index(
                    drop=True
                ),
...
def get_high_tiou_predictions(ground_truth, prediction, tiou_threshold=0.1):
    """Return all predictions with a TIoU score higher than a given threshold.
...
he threshold.
 ...
 # Adaptation to query faster
    ...
    for idx, this_pred in prediction.iterrows():
        ...
        # Check if there is a ground truth with a TIoU score higher than the threshold.
        if np.max(tiou_arr) >= tiou_threshold:
            high_tiou_predictions.append(this_pred)

    # Convert the list of predictions to a data frame.
    high_tiou_predictions = pd.DataFrame(high_tiou_predictions)
    # 定义保存路径
    save_path = './results/high_tiou_predictions.json'

    # 保存为JSON文件
    high_tiou_predictions.to_json(save_path, orient='records', lines=True)


    return high_tiou_predictions
